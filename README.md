...
- **共有ビュー (ウィンドウ共有対応)**: 選んだアプリのウィンドウをキャプチャして字幕を合成した専用ウィンドウを表示。Meet の「ウィンドウ共有」で画面全体を映さずに字幕付きで共有できる (システム標準ピッカーで選択するため画面収録の権限許可は不要)
...
6. Google Meet で共有する場合は次のどちらかを選ぶ (通常のウィンドウ共有にはオーバーレイが映らない):
   ...
   - **ウィンドウ共有 + 共有ビュー**: メニューの「共有ビューを開く…」で映したいアプリのウィンドウを選ぶと、キャプチャ映像に字幕を合成した「Konnyaku 共有ビュー」ウィンドウが開く。Meet ではこのウィンドウを共有する。共有元のアプリはそのまま操作してよい (共有ビューが背面でもキャプチャは継続する)。共有元ウィンドウの最小化だけは映像が止まるため避ける
