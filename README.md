...
6. Google Meet で共有する場合は次のどちらかを選ぶ (通常のウィンドウ共有にはオーバーレイが映らない):
...
- `config` — 入力/出力言語・文字サイズ・低遅延翻訳・リアルタイム翻訳・AI 補正 (`key = value` 形式、行頭 `#` でコメント。インラインコメント非対応)。設定画面からの変更もここへ書き戻され、手編集は次回起動時に反映される
