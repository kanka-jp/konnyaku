...
- **字幕配置 (`subtitle-placement` = overlay / band)**: band は映像の外に字幕専用の黒帯 (高さ = 160 × fontScale) を設け、映像と字幕を非重畳にする。帯の加算定数は `contentAspectRatio` の比率制約で表現できないため、band 中は比率制約を外し `windowWillResize(_:to:)` で「映像部の縦横比 + 帯固定高」を強制する。設定変更は SwiftUI の `onChange` → AppKit レイアウト (映像領域の分割) とウィンドウサイズへ即時反映
- **字幕の表示位置 (`subtitle-position` = bottom / top)**: 配置モードと直交する軸。overlay × top は映像上部に重ね、band × top は黒帯を映像の上に置く (AppKit レイアウトの映像領域 y と SwiftUI 側の整列を同時に切り替え)。帯の高さ・ウィンドウサイズ計算には影響しない
